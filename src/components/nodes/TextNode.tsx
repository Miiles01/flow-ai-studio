            ...
            >
              <div className={`flex items-center gap-0.5 rounded-xl px-2 py-1.5 ${isDark ? 'bg-[#1C1C1E] border border-white/10' : 'bg-white border border-gray-200 '}`}>
                ...
                <div className="relative">
                  ...
                  {activePicker === "bg" && (
                    <div className={`absolute bottom-full mb-2 left-1/2 -translate-x-1/2 rounded-xl p-2.5 grid grid-cols-5 gap-1.5 z-50 w-[150px] ${isDark ? 'bg-[#1C1C1E] border border-white/10' : 'bg-white border border-gray-100'} shadow-sm`}>
              ...
              <AnimatePresence>
                {showLinkInput && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -4 }}
                    transition={{ duration: 0.12 }}
                    className={`absolute top-[calc(100%+6px)] left-1/2 -translate-x-1/2 flex items-center gap-2 rounded-xl px-3 py-2 z-40 ${isDark ? 'bg-[#1C1C1E] border border-white/10' : 'bg-white border border-gray-200 '} shadow-sm`}
                    style={{ minWidth: 280 }}
                    onMouseDown={(e) => e.stopPropagation()}
                  >
