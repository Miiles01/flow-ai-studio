...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const schemaHints: Record<string, string> = {
      ...
      contractsNode: `ContractsNodeData: { title?: string, currency?: "MXN"|"USD"|"EUR"|"COP"|"ARS"|"CLP", pageSize?: "Carta"|"A4"|"Oficio", logoUrl?, logoPosition?, publicId?, pages: [{ id, content }] }. Cada página admite formato ligero: **negritas**, *cursivas*, encabezados con ## , viñetas con "- ", listas numeradas "1. ", sangría con 2 espacios y enlaces [texto](https://...). Nunca uses mayúsculas sostenidas. Marca en **negrita** los campos que el usuario debe completar, escritos entre corchetes, p. ej. **[NOMBRE DE LA MARCA]**, y si te falta un dato dilo en el "answer" explicando qué debe llenar. Estructura el contrato con secciones numeradas: partes, objeto, vigencia, entregables, contraprestación y forma de pago, confidencialidad, propiedad intelectual, terminación, ley aplicable y firmas. Redacta en español neutro, claro y profesional; usa corchetes [así] para los datos que el usuario debe completar. Reparte el contenido en varias páginas: si la información crece, AÑADE más páginas (unas 38-42 líneas por página como máximo) en vez de amontonar todo en una; nunca cortes una frase o una cláusula a la mitad y NO cambies publicId ni logoUrl salvo petición expresa. Si el usuario pide añadir o modificar una cláusula, conserva el resto del texto tal cual.`,
...
Responde SIEMPRE con la tool "widget_result".`;
    ...
    // Si la API Key propia (BYOK) falla por saldo/cuota/credenciales, avisamos al usuario
    // para que cambie de modelo en vez de caer silenciosamente al modelo incluido.
    if (!res.ok && byok && [401, 402, 429].includes(res.status)) {
      return new Response(JSON.stringify({
        error: "Tu modelo propio no tiene créditos disponibles. Cambia a otro modelo en Apps → Modelos.",
        switchModel: true,
